...
def store_dataset(json_filepath, save_dir, num_threads, patch_size, num_loader_threads=1, num_jobs=None, job_index=None):
    ...
    if num_jobs is not None:
        sub_containers_split = split_list(sub_containers, num_jobs)
        sub_containers = sub_containers_split[job_index]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--job_index", required=False, type=int, default=None, help="Job index for parallelization across jobs.")
    args = parser.parse_args()

    store_dataset(args.input, args.output, args.threads, args.patch_size, num_jobs=args.num_jobs, job_index=args.job_index)
